...
function pairTo(nvhttpHost, onSuccess, onFailure, pairingNum) { // 성환수정: pairingNum추가한 인자
    if(!pairingCert) {
    ...
    nvhttpHost.pollServer(function (ret) {
        ...
        var randomNumber = "0000";
        if(pairingNum){
            randomNumber = pairingNum;
        }
        //
...
function hostChosen(host) {
    ...
    console.log("host has been chosen");
    console.log(host);
...
function startGame(host, appID, option) { // 성환수정: option인자 추가
    if(!host || !host.paired) {
    ...
    host.refreshServerInfo().then(function (ret) {
        host.getAppById(appID).then(function (appToStart) {
            ...
            // 성환수정: stream option을 직접하는게 아니라 코드내에서 지정
            if(option){
